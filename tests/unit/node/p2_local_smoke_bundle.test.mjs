...
test('buildP2SmokeSteps returns the expected gateway security, UC-10, UC-11 and UC-14 command sequence', () => {
  ...
  assert.equal(steps[1].name, 'uc10-real-backend-e2e');
  ...
  assert.equal(steps[2].name, 'uc11-real-backend-e2e');
  ...
  assert.equal(steps[3].name, 'uc11-higress-rbac-e2e');
  ...
  assert.equal(steps[4].name, 'uc14-real-backend-e2e');
