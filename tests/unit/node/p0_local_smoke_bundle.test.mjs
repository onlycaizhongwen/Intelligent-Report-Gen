...
test('buildP0SmokeSteps returns the expected UC-01, UC-02, UC-06 and UC-04 command sequence', () => {
  ...
  assert.equal(steps[5].name, 'uc02-report-template-higress-e2e');
  ...
  assert.equal(steps[6].name, 'uc04-higress-export-e2e');
  ...
  assert.equal(steps[7].name, 'uc04-enterprise-export-template-higress-e2e');
  ...
  assert.equal(steps[8].name, 'uc04-real-backend-e2e');
