...
test('buildP3SmokeSteps returns the expected UC-13, UC-07 and UC-08 command sequence', () => {
  ...
  assert.equal(steps[2].name, 'uc08-real-backend-e2e');
  ...
  assert.equal(steps[3].name, 'uc08-rule-runtime-real-backend-e2e');
  ...
  assert.equal(steps[4].name, 'uc08-rule-runtime-higress-compensation-e2e');
  ...
  assert.equal(steps[4].env.REAL_BACKEND_API_BASE_URL, 'http://127.0.0.1:18000/api/v1');
  assert.equal(steps[4].env.REAL_BACKEND_ORIGIN, 'http://127.0.0.1:18000');

  assert.equal(steps[5].name, 'uc08-rule-runtime-higress-replay-exhaustion-e2e');
  ...
  assert.equal(steps[5].env.RULE_WEBHOOK_REPLAY_WORKER_ENABLED, 'true');

  assert.equal(steps[6].name, 'uc08-delegate-rules-real-backend-e2e');
