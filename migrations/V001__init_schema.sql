...
-- Execute with Flyway or psql: psql -d report_ai -f migrations/V001__init_schema.sql
-- Notes: generated from schema.sql; table/index creation uses IF NOT EXISTS for repeatable local execution.
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_file_objects_bucket_key ON file_objects(bucket, object_key) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_file_objects_owner ON file_objects(owner_user_id, storage_purpose) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_outbox_events_event_key ON outbox_events(event_key);
...
CREATE INDEX IF NOT EXISTS idx_users_status_department ON users(status, department) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_roles_code ON roles(role_code) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_permissions_code ON permissions(permission_code);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_user_roles_user_role ON user_roles(user_id, role_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_user_roles_role ON user_roles(role_id) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_role_permissions_role_permission ON role_permissions(role_id, permission_id) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_reports_owner_status ON reports(owner_user_id, status) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_reports_updated_at ON reports(updated_at DESC) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_report_generation_tasks_creator_status ON report_generation_tasks(created_by, status);
CREATE INDEX IF NOT EXISTS idx_report_generation_tasks_report ON report_generation_tasks(report_id);
...
CREATE INDEX IF NOT EXISTS idx_report_outlines_task ON report_outlines(task_id);
...
CREATE INDEX IF NOT EXISTS idx_report_sections_report_version ON report_sections(report_id, version_id, section_no) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_report_versions_report_no ON report_versions(report_id, version_no);
CREATE INDEX IF NOT EXISTS idx_report_versions_current ON report_versions(report_id, is_current);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_report_templates_code ON report_templates(template_code) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_citation_sources_report ON citation_sources(report_id);
...
CREATE INDEX IF NOT EXISTS idx_citation_anchors_report_section ON citation_anchors(report_id, section_id);
CREATE INDEX IF NOT EXISTS idx_citation_anchors_source ON citation_anchors(citation_source_id);
...
CREATE INDEX IF NOT EXISTS idx_citation_quality_source ON citation_quality_scores(citation_source_id);
...
CREATE INDEX IF NOT EXISTS idx_export_files_report_version ON export_files(report_id, version_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_export_files_status ON export_files(export_status, created_at);
...
CREATE INDEX IF NOT EXISTS idx_knowledge_bases_owner ON knowledge_bases(owner_user_id, status) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_knowledge_entries_base_status ON knowledge_entries(knowledge_base_id, index_status) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_knowledge_entries_title ON knowledge_entries USING gin(to_tsvector('simple', title)) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_knowledge_documents_base_status ON knowledge_documents(knowledge_base_id, parse_status) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_document_parse_results_doc ON document_parse_results(document_id, parse_type);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_document_chunks_doc_index ON document_chunks(document_id, chunk_index) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_document_chunks_entry ON document_chunks(knowledge_entry_id) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_embeddings_chunk_model ON embeddings(chunk_id, embedding_model);
CREATE INDEX IF NOT EXISTS idx_embeddings_milvus_key ON embeddings(milvus_collection, milvus_primary_key);
...
CREATE INDEX IF NOT EXISTS idx_data_source_connections_base_status ON data_source_connections(knowledge_base_id, status) WHERE deleted_at IS NULL;

CREATE TABLE IF NOT EXISTS data_source_sync_tasks (
  id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_data_source_sync_tasks_source_status ON data_source_sync_tasks(data_source_id, status, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_retrieval_sessions_task ON retrieval_sessions(task_id);
...
CREATE INDEX IF NOT EXISTS idx_retrieval_results_session_rank ON retrieval_results(session_id, rank_no);
...
CREATE INDEX IF NOT EXISTS idx_context_segments_session_order ON context_segments(session_id, segment_order);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_prompt_templates_code_version ON prompt_templates(template_code, version_no) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_model_invocations_event_key ON model_invocations(audit_event_key);
CREATE INDEX IF NOT EXISTS idx_model_invocations_task ON model_invocations(task_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_model_invocations_model ON model_invocations(provider, model_name, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_model_responses_invocation ON model_responses(invocation_id, response_order);
...
CREATE INDEX IF NOT EXISTS idx_rules_creator_status ON rules(created_by, status) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_rule_nodes_rule_key ON rule_nodes(rule_id, node_key) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_rule_edges_rule_key ON rule_edges(rule_id, edge_key) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_rule_versions_rule_no ON rule_versions(rule_id, version_no);
...
CREATE INDEX IF NOT EXISTS idx_rule_execution_records_rule ON rule_execution_records(rule_id, created_at DESC);

CREATE TABLE IF NOT EXISTS rule_node_execution_logs (
  id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_rule_node_logs_execution ON rule_node_execution_logs(execution_id, created_at);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_share_links_token ON share_links(share_token);
CREATE INDEX IF NOT EXISTS idx_share_links_report ON share_links(report_id, expires_at);

CREATE TABLE IF NOT EXISTS share_access_records (
  id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_share_access_records_link_time ON share_access_records(share_link_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_annotations_report ON annotations(report_id, status) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_annotation_comments_annotation ON annotation_comments(annotation_id, created_at) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_collaboration_tasks_assignee ON collaboration_tasks(assignee_id, status) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_notifications_receiver ON notifications(receiver_user_id, read_at, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_operation_logs_operator_time ON operation_logs(operator_user_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_operation_log_details_log ON operation_log_details(operation_log_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_dashboard_metric_snapshots_date_scope ON dashboard_metric_snapshots(metric_date, metric_scope);

CREATE TABLE IF NOT EXISTS dashboard_trend_points (
  id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_dashboard_trend_points_name_date ON dashboard_trend_points(metric_name, metric_date);

CREATE TABLE IF NOT EXISTS dashboard_rank_items (
  id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_dashboard_rank_items_type_date ON dashboard_rank_items(rank_type, rank_date, rank_no);
...
CREATE INDEX IF NOT EXISTS idx_dashboard_activities_time ON dashboard_activities(activity_time DESC);
