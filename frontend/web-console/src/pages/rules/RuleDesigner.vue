    ...
    <section class="rule-layout">
      <aside class="rule-list">
        ...
        >
          ...
          <span>{{ rule.status }} · v{{ rule.versionId }}</span>
        </button>
      </aside>
        ...
        <template v-if="selectedRule">
          <section class="flow-canvas-panel">
            <div class="section-heading">
              <h3>Interactive rule canvas</h3>
              <span class="flow-hint">Drag nodes to arrange the execution flow.</span>
            </div>
          ...
          <section class="node-grid" aria-label="Rule node summary list">
            <article v-for="node in nodes" :key="node.id" class="node-card">
              ...
              <p v-else-if="node.type === 'approval'">
                approval {{ approvalNodeMode(node) }} {{ approvalNodeRoles(node).join(', ') }} -> {{ node.approvalTitle }}
              </p>
              ...
              <p v-else-if="node.type === 'action' && node.actionType === 'create_task'">
                task report {{ node.reportId }} -> user {{ node.assigneeUserId }}
              </p>
            </article>
          </section>
          ...
          <section class="edge-list">
            ...
            >
              <template v-if="topologyIssueEdgeKeys.includes(edgeKey(edge))">Broken path </template>{{ edge.source }} -> {{ edge.target }}
            </span>
          </section>

          <section class="canvas-editor">
            <div class="section-heading">
              <h3>Rule canvas editor</h3>
              <el-button type="primary" :loading="savingCanvas" @click="saveCanvasChanges">Save canvas changes</el-button>
            </div>
            <div class="template-apply-bar" aria-label="Approval template application">
              <label class="field">
                <span>Approval template</span>
                <select v-model="selectedApprovalTemplateId" aria-label="Approval template selector">
                  <option value="">Select reusable approval template</option>
                  <option
              ...
              <el-button :disabled="!selectedApprovalTemplateId" @click="applyApprovalTemplate">Apply approval template</el-button>
              <label class="field">
                <span>Insert after node</span>
                <select v-model="approvalTemplateInsertAfterNodeId" aria-label="Insert approval template after node">
                  <option value="">Append to canvas</option>
              ...
              <label v-if="approvalTemplateInsertCandidateEdges.length > 1" class="field">
                ...
                <select v-model="approvalTemplateInsertEdgeKey" aria-label="Insert approval template into branch path">
                  <option value="">All outgoing paths</option>
                  <option
              ...
              <div v-if="approvalTemplateEdgePreviewItems.length > 0" class="template-edge-preview" aria-label="Approval template edge change preview">
                <span
                  v-for="item in approvalTemplateEdgePreviewItems"
                  :key="`${item.changeType}-${item.source}-${item.target}-${item.condition ?? ''}`"
                >
                  {{ item.changeType === 'remove' ? 'Remove edge' : 'Add edge' }} {{ edgePreviewLabel(item) }}
                </span>
              </div>
            </div>
            <div class="config-grid">
              <label class="field">
                <span>New node id</span>
                <el-input v-model="newNodeForm.id" aria-label="New node id" />
              </label>
              <label class="field">
                <span>New node type</span>
                <select v-model="newNodeForm.type" aria-label="New node type">
              ...
              <label class="field" v-if="newNodeForm.type === 'action'">
                <span>New action type</span>
                <el-input v-model="newNodeForm.actionType" aria-label="New action type" />
              </label>
              <label class="field" v-if="newNodeForm.type === 'action'">
                <span>New action message</span>
                <el-input v-model="newNodeForm.message" aria-label="New action message" />
              </label>
              <template v-if="newNodeForm.type === 'action' && newNodeForm.actionType === 'create_task'">
                <label class="field">
                  <span>New task report</span>
                  <select v-model="newNodeForm.reportId" aria-label="New task report selector" @change="loadSelectedTaskReportDetail">
                    <option value="">Manual report id</option>
                ...
                <label class="field">
                  <span>New task report id</span>
                  <el-input v-model="newNodeForm.reportId" aria-label="New task report id" />
                </label>
                <label class="field">
                  <span>New task assignee</span>
                  <select v-model="newNodeForm.assigneeUserId" aria-label="New task assignee selector">
                    <option value="">Manual assignee user id</option>
                ...
                <label class="field">
                  <span>New task assignee user id</span>
                  <el-input v-model="newNodeForm.assigneeUserId" aria-label="New task assignee user id" />
                </label>
                <label class="field">
                  <span>New task content</span>
                  <el-input v-model="newNodeForm.content" aria-label="New task content" />
                </label>
                <label class="field">
                  <span>New task section id</span>
                  <el-input v-model="newNodeForm.sectionId" aria-label="New task section id" />
                </label>
                <label class="field">
                  <span>New task start offset</span>
                  <el-input v-model="newNodeForm.startOffset" aria-label="New task start offset" />
                </label>
                <label class="field">
                  <span>New task end offset</span>
                  <el-input v-model="newNodeForm.endOffset" aria-label="New task end offset" />
                </label>
                <label class="field">
                  <span>New task selected text</span>
                  <el-input v-model="newNodeForm.selectedText" aria-label="New task selected text" />
                </label>
                <div v-if="taskReportSections.length > 0" class="field field-wide task-anchor-picker">
                  <div class="section-heading compact">
                    <h4>Task anchor source</h4>
                    <el-button size="small" @click="applySelectedTextAsTaskAnchor">Use selected text as task anchor</el-button>
                  </div>
                  <article
              ...
              <template v-if="newNodeForm.type === 'condition' || newNodeForm.type === 'branch'">
                <label class="field">
                  <span>New condition field</span>
                  <el-input v-model="newNodeForm.field" aria-label="New condition field" />
                </label>
                <label class="field">
                  <span>New condition operator</span>
                  <el-input v-model="newNodeForm.operator" aria-label="New condition operator" />
                </label>
                <label class="field">
                  <span>New condition value</span>
                  <el-input v-model="newNodeForm.value" aria-label="New condition value" />
                </label>
              </template>
              <template v-if="newNodeForm.type === 'aggregate'">
                <label class="field">
                  <span>New aggregate source field</span>
                  <el-input v-model="newNodeForm.sourceField" aria-label="New aggregate source field" />
                </label>
                <label class="field">
                  <span>New aggregate operation</span>
                  <select v-model="newNodeForm.operation" aria-label="New aggregate operation">
                    <option value="sum">sum</option>
                    <option value="count">count</option>
                  </select>
                </label>
                <label class="field">
                  <span>New aggregate value field</span>
                  <el-input v-model="newNodeForm.valueField" aria-label="New aggregate value field" />
                </label>
                <label class="field">
                  <span>New aggregate output field</span>
                  <el-input v-model="newNodeForm.outputField" aria-label="New aggregate output field" />
                </label>
              </template>
              <template v-if="newNodeForm.type === 'subprocess'">
                <label class="field">
                  <span>New subprocess rule id</span>
                  <el-input v-model="newNodeForm.subprocessRuleId" aria-label="New subprocess rule id" />
                </label>
                <label class="field">
                  <span>New subprocess name</span>
                  <el-input v-model="newNodeForm.subprocessName" aria-label="New subprocess name" />
                </label>
              </template>
              <template v-if="newNodeForm.type === 'approval'">
                <label class="field">
                  <span>New approval assignee organization role</span>
                  <el-select
                    v-model="selectedApprovalAssigneeRole"
                    aria-label="New approval assignee organization role"
                    filterable
                ...
                <label class="field">
                  <span>New approval assignee roles</span>
                  <el-input v-model="newNodeForm.assigneeRoles" aria-label="New approval assignee roles" placeholder="finance_manager, legal_manager" />
                </label>
                <label class="field">
                  <span>New approval mode</span>
                  <select v-model="newNodeForm.approvalMode" aria-label="New approval mode">
                    <option value="all">all</option>
                    <option value="any">any</option>
                  </select>
                </label>
                <label class="field">
                  <span>New approval delegate role</span>
                  <el-select
                    v-model="newNodeForm.delegateRole"
                    aria-label="New approval delegate organization role"
                    filterable
                    class="role-directory-select"
                    placeholder="Select from organization directory"
                  >
                  ...
                  <el-input v-model="newNodeForm.delegateRole" aria-label="New approval delegate role" placeholder="finance_delegate" />
                </label>
                <label class="field">
                  <span>New approval delegate active from</span>
                  <el-input v-model="newNodeForm.delegateActiveFrom" aria-label="New approval delegate active from" placeholder="2026-06-26T08:00:00Z" />
                </label>
                <label class="field">
                  <span>New approval delegate active to</span>
                  <el-input v-model="newNodeForm.delegateActiveTo" aria-label="New approval delegate active to" placeholder="2026-06-26T18:00:00Z" />
                </label>
                <label class="field">
                  <span>New approval title</span>
                  <el-input v-model="newNodeForm.approvalTitle" aria-label="New approval title" />
                </label>
                <label class="field">
                  <span>New approval SLA hours</span>
                  <el-input v-model="newNodeForm.slaHours" aria-label="New approval SLA hours" />
                </label>
                <label class="field">
                  <span>New approval SLA escalation role</span>
                  <el-input v-model="newNodeForm.slaEscalationRole" aria-label="New approval SLA escalation role" placeholder="finance_director" />
                </label>
                <label class="field">
                  <span>New approval SLA escalation policies</span>
                  <el-input v-model="newNodeForm.slaEscalations" aria-label="New approval SLA escalation policies" placeholder="4:finance_director, 8:risk_vp" />
                </label>
              </template>
            </div>
            <div class="editor-actions">
              <el-button @click="addCanvasNode">Add node</el-button>
              <label class="field">
                <span>Delete node</span>
                <select v-model="deleteNodeId" aria-label="Delete node">
                  <option value="">Select node</option>
                  <option v-for="node in deletableNodes" :key="node.id" :value="node.id">{{ node.id }}</option>
                </select>
              </label>
              <el-button :disabled="!deleteNodeId" @click="deleteCanvasNode">Delete node</el-button>
            </div>
            <div class="editor-actions">
              <label class="field">
                <span>New edge source</span>
                <select v-model="newEdgeForm.source" aria-label="New edge source">
                  <option value="">Select source</option>
                  <option v-for="node in nodes" :key="node.id" :value="node.id">{{ node.id }}</option>
                </select>
              </label>
              <label class="field">
                <span>New edge target</span>
                <select v-model="newEdgeForm.target" aria-label="New edge target">
                  <option value="">Select target</option>
                  <option v-for="node in nodes" :key="node.id" :value="node.id">{{ node.id }}</option>
                </select>
              </label>
              <label class="field">
                <span>New edge condition</span>
                <el-input v-model="newEdgeForm.condition" aria-label="New edge condition" />
              </label>
              <el-button @click="addCanvasEdge">Add edge</el-button>
              <label class="field">
                <span>Delete edge</span>
                <select v-model="deleteEdgeKey" aria-label="Delete edge">
                  <option value="">Select edge</option>
                  <option v-for="edge in deletableEdges" :key="edge.key" :value="edge.key">{{ edge.label }}</option>
                </select>
              </label>
              <el-button :disabled="!deleteEdgeKey" @click="deleteCanvasEdge">Delete edge</el-button>
            </div>
          </section>

          <section v-if="webhookActionNodes.length > 0" class="webhook-config">
            <div class="section-heading">
              <h3>Webhook node configuration</h3>
              <el-button type="primary" :loading="savingWebhookConfig" @click="saveWebhookConfig">Save webhook config</el-button>
            </div>
            <label class="field">
              <span>Webhook node</span>
              <select v-model="selectedWebhookNodeId" aria-label="Webhook node" @change="syncWebhookConfigFromSelectedRule(selectedWebhookNodeId)">
            ...
            <div class="config-grid">
              <label class="field">
                ...
                <el-input v-model="webhookConfig.endpoint" aria-label="Webhook endpoint" />
              </label>
              <label class="field">
                <span>Webhook method</span>
                <el-input v-model="webhookConfig.method" aria-label="Webhook method" />
              </label>
              <label class="field">
                <span>Webhook max retry count</span>
                <el-input v-model="webhookConfig.maxRetryCount" aria-label="Webhook max retry count" />
              </label>
              <label class="field">
                <span>Webhook retry backoff seconds</span>
                <el-input v-model="webhookConfig.retryBackoffSeconds" aria-label="Webhook retry backoff seconds" />
              </label>
              <label class="field">
                <span>Webhook max async replay attempts</span>
                <el-input v-model="webhookConfig.maxAsyncReplayAttempts" aria-label="Webhook max async replay attempts" />
              </label>
              <label class="field">
                <span>Webhook signature secret</span>
                <el-input v-model="webhookConfig.signatureSecret" aria-label="Webhook signature secret" type="password" show-password />
              </label>
              <label class="field field-wide">
                <span>Webhook headers JSON</span>
                <el-input v-model="webhookConfig.headersJson" type="textarea" :rows="4" aria-label="Webhook headers JSON" />
              </label>
              <label class="field field-wide">
                <span>Webhook body JSON</span>
                <el-input v-model="webhookConfig.bodyJson" type="textarea" :rows="4" aria-label="Webhook body JSON" />
              </label>
            </div>
          </section>
          ...
          <section v-if="debugResult" class="debug-result">
            ...
            <p>evaluatedNodes: {{ debugResult.output.evaluatedNodes }}</p>
          ...
          <section v-if="productionResult" class="debug-result">
            ...
            <p>runType: {{ productionResult.runType }} · versionId: {{ productionResult.versionId }}</p>
          </section>
          ...
          <section class="debug-result">
            ...
            <div class="trace-list">
              <span v-for="run in runHistory" :key="run.runId" class="run-history-row">
                #{{ run.runId }} 路 {{ run.runType }} 路 {{ run.status }} 路 {{ run.matched ? 'true' : 'false' }}
              </span>
            </div>
          </section>
          ...
          <section v-if="subprocessTopology" class="debug-result subprocess-topology">
            <h3>Subprocess topology</h3>
            <div class="topology-list" aria-label="Subprocess topology nodes">
              <span v-for="node in subprocessTopology.nodes" :key="`${node.role}-${node.runId}`">
                {{ node.role }} run {{ node.runId }} · rule {{ node.ruleId }} · {{ node.status }}
              </span>
            </div>
            <div class="topology-list" aria-label="Subprocess topology edges">
          ...
          <section class="debug-result">
            ...
            <div v-if="approvalRecords.length > 0" class="approval-record-list">
              <article v-for="record in approvalRecords" :key="record.approvalRecordId" class="approval-record-row">
                <div>
                  <strong>{{ record.nodeId }} 路 {{ record.assigneeRole }} 路 {{ record.status }}</strong>
            ...
            <el-empty v-else description="No approval records" />
          </section>
          <section class="debug-result action-ledger">
            <div class="section-heading">
              <h3>Webhook action ledger</h3>
              <div class="ledger-actions">
                ...
                <el-button :loading="loadingActionExecutions" @click="loadActionExecutions">Refresh</el-button>
              </div>
            </div>
            <section class="compensation-summary" aria-label="Compensation operations">
              <h4>Compensation operations</h4>
              <span>Pending {{ actionCompensationSummary.pending }}</span>
              <span>Succeeded {{ actionCompensationSummary.succeeded }}</span>
              <span>Exhausted {{ actionCompensationSummary.exhausted }}</span>
              <span>Ignored {{ actionCompensationSummary.ignored }}</span>
              <strong>Success rate {{ actionCompensationSummary.successRate }}%</strong>
            </section>
            <div v-if="filteredActionExecutions.length > 0" class="ledger-list">
              <article v-for="execution in filteredActionExecutions" :key="execution.actionExecutionId" class="ledger-row">
                <el-checkbox
                  ...
                  :aria-label="`Select action ${execution.actionExecutionId}`"
                ...
                <div>
                  ...
                  <small v-if="execution.errorMessage">Failure: {{ execution.errorMessage }}</small>
                  <small v-if="execution.sourceActionExecutionId">Source action #{{ execution.sourceActionExecutionId }}</small>
                  <span>{{ execution.status }} · attempt {{ execution.attempt }}/{{ execution.maxRetryCount }}</span>
                </div>
                ...
                >
                  Retry {{ execution.actionExecutionId }}
                </el-button>
              </article>
            </div>
            <el-empty v-else description="No webhook action executions" />
          </section>
        </template>
...
async function loadSelectedTaskReportDetail() {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : 'Task report detail load failed';
  }
}
...
function applySelectedTextAsTaskAnchor() {
  ...
  if (!sectionElement || !selectedText || startOffset < 0) {
    errorMessage.value = 'Select report text before applying task anchor';
    return;
  }
  ...
  return definitionNodes.map((node, index) => {
    ...
    return {
      ...
      data: {
        ...
        topologyStatus,
        topologyLabel: topologyStatus ? 'Topology blocked' : undefined
      }
    };
  });
}
...
function approvalTemplatePreviewStatusLabel(status: NonNullable<RuleFlowNodeData['previewStatus']>) {
  if (status === 'insert-point') return 'Preview insert point';
  if (status === 'target') return 'Preview target';
  return 'Preview approval step';
}
...
function buildFlowEdges(definitionEdges: RuleEdge[]) {
  ...
  return definitionEdges.map((edge) => ({
    ...
    source: edge.source,
    target: edge.target,
    label: brokenEdgeKeys.has(edgeKey(edge)) ? `Broken path ${edge.source} -> ${edge.target}` : edge.condition ?? '',
...
function approvalTemplateFlowEdgePreviewLabel(item: ApprovalTemplateEdgePreviewItem) {
  return `Preview ${item.changeType === 'remove' ? 'remove' : 'add'} edge ${item.source} -> ${item.target}`;
}
...
function nodeSummary(node: RuleNode) {
  ...
  if (node.type === 'subprocess') {
    return `subprocess ${node.subprocessRuleId}${node.subprocessName ? ` -> ${node.subprocessName}` : ''}`;
  }
  if (node.type === 'approval') {
    const delegateText = node.delegateRole ? ` delegate ${node.delegateRole}` : '';
    const delegateWindowText = node.delegateActiveFrom || node.delegateActiveTo
      ? ` ${node.delegateActiveFrom || '*'}..${node.delegateActiveTo || '*'}`
      : '';
    const escalationText = node.slaEscalationRole ? ` escalate ${node.slaEscalationRole}` : '';
    const escalationPolicyText = node.slaEscalations?.length
      ? ` policies ${node.slaEscalations.map((policy) => `${policy.afterHours}h:${policy.role}`).join(', ')}`
      : '';
    return `approval ${approvalNodeMode(node)} ${approvalNodeRoles(node).join(', ')}${delegateText}${delegateWindowText}${escalationText}${escalationPolicyText} -> ${node.approvalTitle}`;
  }
  ...
  if (node.type === 'action' && node.actionType === 'create_task') {
    return `task report ${node.reportId} -> user ${node.assigneeUserId}`;
  }
  if (node.type === 'action') {
...
function validateRuleTopology(definition: RuleDefinition): RuleTopologyValidationResult | null {
  ...
  if (!startNode || !definitionNodes.some((node) => node.type === 'end')) {
    return { message: 'Rule topology requires start and end nodes', blockedNodeIds: startNode ? [startNode.id] : [], blockedEdgeKeys: [] };
  }
  ...
  for (const edge of definitionEdges) {
    if (!nodeTypes.has(edge.source) || !nodeTypes.has(edge.target) || edge.source === edge.target) {
      return {
        message: 'Rule topology contains an invalid edge',
  ...
  const validatePath = (nodeId: string, path: Set<string>): RuleTopologyValidationResult | null => {
    ...
    if (!nodeType) {
      return { message: 'Rule topology contains an invalid edge', blockedNodeIds: [], blockedEdgeKeys: [] };
    }
    if (path.has(nodeId)) {
      return { message: 'Rule topology contains a cycle', blockedNodeIds: [nodeId], blockedEdgeKeys: [] };
    }
    if (nodeType === 'end') {
      return null;
    }
    ...
    if (outgoingEdges.length === 0) {
      return { message: 'Rule topology path cannot reach end', blockedNodeIds: [nodeId], blockedEdgeKeys: [] };
    }
...
async function loadSubprocessTopology(run: RuleRunHistoryItem) {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : 'Subprocess topology load failed';
  } finally {
...
async function loadApprovalRecords() {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : 'Approval records load failed';
  }
}
...
async function handleApprovalRecord(record: RuleApprovalRecordItem, action: 'approve' | 'reject') {
  ...
  try {
    await ruleApi.handleApprovalRecord(String(selectedRule.value.ruleId), String(record.approvalRecordId), {
      action,
      comment: action === 'approve' ? 'approved from rule page' : 'rejected from rule page'
    });
    await loadApprovalRecords();
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : `Approval ${action} failed`;
  } finally {
...
async function loadActionExecutions() {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : 'Webhook action ledger load failed';
  } finally {
...
async function retryActionExecution(execution: RuleActionExecutionItem) {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : 'Webhook action retry failed';
  } finally {
...
async function batchHandleActionExecutions(operation: 'retry' | 'ignore') {
  ...
  try {
    await ruleApi.batchHandleWebhookActionExecutions(String(selectedRule.value.ruleId), {
      ...
      reason: operation === 'ignore' ? 'ignored from rule action ledger' : undefined
    });
    selectedActionExecutionIds.value = [];
    await loadActionExecutions();
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : `Webhook action batch ${operation} failed`;
  } finally {
...
function applyApprovalTemplate() {
  ...
  if (!template) {
    errorMessage.value = 'Approval template is required';
    return;
  }
  const prefix = `tpl${template.approvalTemplateId}_`;
  ...
  if (duplicated) {
    errorMessage.value = `Approval template node already exists: ${duplicated.id}`;
    return;
  }
...
function addCanvasNode() {
  ...
  if (!id) {
    errorMessage.value = 'New node id is required';
    return;
  }
  if (nodes.value.some((node) => node.id === id)) {
    errorMessage.value = 'New node id already exists';
    return;
  }
  const node: RuleNode = {
    id,
  ...
  if (newNodeForm.value.type === 'action') {
    ...
    if (node.actionType === 'create_task') {
      ...
      if (reportId === null) {
        errorMessage.value = 'New task report id is required';
        return;
      }
      if (assigneeUserId === null) {
        errorMessage.value = 'New task assignee user id is required';
        return;
      }
      if (!content) {
        errorMessage.value = 'New task content is required';
        return;
      }
      if (startOffset === null) {
        errorMessage.value = 'New task start offset is required';
        return;
      }
      if (endOffset === null || endOffset <= startOffset) {
        errorMessage.value = 'New task end offset must be greater than start offset';
        return;
      }
      if (!selectedText) {
        errorMessage.value = 'New task selected text is required';
        return;
      }
  ...
  if (newNodeForm.value.type === 'condition' || newNodeForm.value.type === 'branch') {
    ...
    if (!field) {
      errorMessage.value = 'New condition field is required';
      return;
    }
    if (!operator) {
      errorMessage.value = 'New condition operator is required';
      return;
    }
    if (!value) {
      errorMessage.value = 'New condition value is required';
      return;
    }
    node.field = field;
  ...
  if (newNodeForm.value.type === 'aggregate') {
    ...
    if (!sourceField) {
      errorMessage.value = 'New aggregate source field is required';
      return;
    }
    if (newNodeForm.value.operation === 'sum' && !valueField) {
      errorMessage.value = 'New aggregate value field is required';
      return;
    }
    if (!outputField) {
      errorMessage.value = 'New aggregate output field is required';
      return;
    }
  ...
  if (newNodeForm.value.type === 'subprocess') {
    ...
    if (subprocessRuleId === null) {
      errorMessage.value = 'New subprocess rule id is required';
      return;
    }
  ...
  if (newNodeForm.value.type === 'approval') {
    ...
    if (assigneeRoles.length === 0) {
      errorMessage.value = 'New approval assignee roles are required';
      return;
    }
    if (!approvalTitle) {
      errorMessage.value = 'New approval title is required';
      return;
    }
    if (slaEscalations === null) {
      errorMessage.value = 'New approval SLA escalation policies must use afterHours:role';
      return;
    }
...
function addCanvasEdge() {
  ...
  if (!source || !target) {
    errorMessage.value = 'New edge source and target are required';
    return;
  }
  if (source === target) {
    errorMessage.value = 'New edge source and target must be different';
    return;
  }
...
async function saveCanvasChanges() {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : 'Rule canvas save failed';
  } finally {
...
async function saveWebhookConfig() {
  ...
  errorMessage.value = '';
  try {
    const headers = parseJsonObject(webhookConfig.value.headersJson, 'Webhook headers JSON');
    const body = parseJsonObject(webhookConfig.value.bodyJson, 'Webhook body JSON');
    ...
    errorMessage.value = error instanceof Error ? error.message : 'Webhook config save failed';
  } finally {
