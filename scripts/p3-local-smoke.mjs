...
import { buildP3SmokeSteps } from './p3-local-smoke-lib.mjs';
...
async function main() {
  const steps = buildP3SmokeSteps({
    realBackendApiBaseUrl:
      process.env.P3_SMOKE_REAL_BACKEND_API_BASE_URL ?? 'http://127.0.0.1:18082/api/v1',
    realBackendOrigin:
      process.env.P3_SMOKE_REAL_BACKEND_ORIGIN ?? 'http://127.0.0.1:18082',
    gatewayApiBaseUrl:
      process.env.P3_SMOKE_HIGRESS_API_BASE_URL ?? 'http://127.0.0.1:18000/api/v1',
    gatewayOrigin:
      process.env.P3_SMOKE_HIGRESS_ORIGIN ?? 'http://127.0.0.1:18000',
  });
