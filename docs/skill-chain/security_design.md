...
| Higress local test IdP smoke | `scripts/higress-oidc-local-smoke.mjs` starts a temporary JWKS endpoint and isolated Java `RS256` runtime, routes through Higress, verifies accepted issuer/audience plus wrong issuer/audience rejection, then restores the default local route. This is local verification evidence only; customer production IdP configuration remains a separate gate. |
...
| Higress trusted TLS smoke | Local HTTPS route smoke may use self-signed certificates with verification disabled; production TLS evidence must run `scripts/higress-tls-certificate-smoke.mjs` with verification enabled and pass trusted-chain plus minimum-validity checks. Customer/private CA chains are supported through `HIGRESS_TLS_CA_FILE`; verification must stay enabled. |
