  ...
  },
  {
    ...
    "requestBody": {
      "file": {
        "type": "binary",
        "required": true,
        "description": "supplement attachment file; default max 10485760 bytes and evidence content types: application/pdf, image/jpeg, image/png, text/csv, text/markdown, text/plain, application/msword, application/vnd.ms-excel, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.openxmlformats-officedocument.wordprocessingml.document; customer deployments may override the backend policy with RULE_APPROVAL_SUPPLEMENT_ATTACHMENT_MAX_SIZE_BYTES and RULE_APPROVAL_SUPPLEMENT_ATTACHMENT_ALLOWED_CONTENT_TYPES; backend rejects known EICAR antivirus test signatures, declared binary content types whose file signatures do not match, PDF uploads containing active content markers such as JavaScript/OpenAction/Launch/EmbeddedFile/RichMedia payloads, image uploads containing active content markers such as script/svg/html/javascript payloads, legacy Office CFB uploads containing VBA/macro markers, OOXML Office archives containing EICAR signatures, macro payloads, embedded object/package/ActiveX/control entries, OOXML Office archives exceeding the 10 MiB expansion inspection limit, OOXML Office archives declaring encrypted ZIP entries, OOXML Office archives containing external relationship targets, and optional external AV/ClamAV INSTREAM malware responses before object storage writes; external AV is disabled by default unless RULE_APPROVAL_SUPPLEMENT_ATTACHMENT_EXTERNAL_AV_ENABLED=true"
      }
    },
    "responseBody": {
      "code": {
